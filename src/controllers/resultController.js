...
const ResultController = {
  getElectionResult: async (req, res) => {
    try {
      ...
      // Step 5: Calculate participation (number of ballots)
      const participation = ballots.length; // Total number of ballots cast

      // Step 6: Insert results into the database
      ...
      res.status(200).json({
        ...
        participation, // Include participation in the response
        winner: {
          ...winner,
          votes: winner.votes.toString(),
          percentage: winner.percentage.toString()
        },
        resultId
      });
    } catch (error) {
